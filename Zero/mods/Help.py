...
class Help:
    ...
    async def help(self, ctx, *commands : str):
        ...
        =help <Command Name>
        =help <Category Name>
        =help <Command Group Name> <Subommand Name>"""
        def repl(obj):
        ...
        if my.permissions_in(channel).embed_links:
            ...
            if len(pages) == 1:
                ...
                page_no = 1
                about = "Zero is a friendly moderation bot that has all sorts of commands."
                colour = ''.join([randchoice('0123456789ABCDEF') for x in range(6)])
                colour = int(colour, 16)
                await self.bot.send_message(ctx.message.author, embed = discord.Embed(title = "About Zero", description = about, colour=0x90181e, timestamp = ctx.message.timestamp))
                for page in pages:
