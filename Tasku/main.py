...
async def on_ready():
    ...
    print(bot.user.name)
    print(bot.user.id)
    await bot.change_presence(game=discord.Game(name="FAQ for you"), status=discord.Status.dnd)
    print('------')
    ...
    async def uptime(self, ctx):
        ...
        try:
            if "<@" + ctx.message.author.id + ">" in open('mods/utils/txt/blacklist.txt').read():
                ...
                d, h = divmod(h, 24)
                w, d = divmod(d, 7)
                await self.bot.say("FAQ has been up for %d Weeks," % (w) + " %d Days," % (d) + " %d Hours,"
                                   % (
